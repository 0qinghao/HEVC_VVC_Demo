...
while 1
    ...
    if (time_hevc >= sum(enc_time_hevc(1:i_hevc + 1)))
        ...
        imshow(RGB(:, :, :, i_hevc))
        subplot(2, 4, 4)
        set(gca, 'Color', 'None','XColor', 'white', 'YColor', 'white', 'xtick', [], 'xticklabel', [], 'ytick', [], 'yticklabel', [])
        delete(t_hevc);
        ...
        t_hevc = text(0, 0.5, text_hevc, 'FontSize', 14, 'FontName', 'courier', 'FontWeight', 'bold');
        i_hevc = i_hevc + 1;
    end
    ...
    if (time_vvc >= sum(enc_time_vvc(1:i_vvc + 1)))
        ...
        t_vvc = text(0, 0.5, text_vvc, 'FontSize', 14, 'FontName', 'courier', 'FontWeight', 'bold');
        i_vvc = i_vvc + 1;
    end
