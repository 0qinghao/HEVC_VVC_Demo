...
while 1
    ...
    if (time_hevc >= sum(enc_time_hevc(1:i_hevc + 1)))
        ...
        delete(t_hevc);
        text_hevc{1} = 'H.265+Proposed Algorithm';
        text_hevc{2} = 'vs';
        text_hevc{3} = 'H.265 Standard';
        text_hevc{4} = ' ';
        text_hevc{5} = strcat("          Frame: ", num2str(i_hevc));
        text_hevc{6} = strcat("       Enc Time: ", num2str(hevc_EI_bits_time(i_hevc,2)/hevc_src_bits_time(i_hevc,2)*100),"%");
        text_hevc{7} = strcat("Bit-rate Saving: ", num2str((hevc_src_bits_time(i_hevc,1)-hevc_EI_bits_time(i_hevc,1))/hevc_src_bits_time(i_hevc,1)*100),"%");
        t_hevc = text(0, 0.8, text_hevc, 'FontSize', 15, 'FontName', "courier");
        i_hevc = i_hevc + 1;
    end
