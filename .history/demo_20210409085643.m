...
waitbar_vvc = waitbar(0, strcat('(H.266/VVC) Encoding Frame ', num2str(i_vvc)), 'Name', 'Encoding (H.265/HEVC)');
...
while 1
    ...
    waitbar((time_vvc - sum(enc_time_vvc(1:i_vvc))) / enc_time_vvc(i_vvc + 1), waitbar_vvc, strcat('(H.266/VVC) Encoding Frame ', num2str(i_vvc)), 'Name', 'Encoding (H.265/HEVC)')
    ...
    if (time_hevc >= sum(enc_time_hevc(1:i_hevc + 1)))
        ...
        delete(t_hevc);
        t_hevc = text(0.5, 0.5, num2str(i_hevc));
        waitbar(0, waitbar_hevc, strcat('(H.265/HEVC) Encoding Frame ', num2str(i_hevc)))
        i_hevc = i_hevc + 1;
    end
    ...
    if (time_vvc >= sum(enc_time_vvc(1:i_vvc + 1)))
        ...
        waitbar(0, waitbar_vvc, strcat('(H.266/VVC) Encoding Frame ', num2str(i_vvc)))
        i_vvc = i_vvc + 1;
    end
