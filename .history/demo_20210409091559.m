...
while 1
    ...
    if (time_hevc >= sum(enc_time_hevc(1:i_hevc + 1)))
        ...
        text_hevc{2} = strcat("Frame:", num2str(i_hevc));
        text_hevc{3} = strcat("Enc Time:", num2str(hevc_EI_bits_time(i_hevc,2)/hevc_src_bits_time(i_hevc,2)*100),"%");
        text_hevc{4} = strcat("Bit-rate Saving:", num2str((hevc_src_bits_time(i_hevc,1)-hevc_EI_bits_time(i_hevc,1))/hevc_src_bits_time(i_hevc,1)*100),"%");
